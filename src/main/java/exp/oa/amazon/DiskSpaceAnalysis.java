...
/**
 * Created by Jim Z on 2020-09-24 18:56
 *
...
public class DiskSpaceAnalysis {
    public int maxOfSegment(int numComputer, List<Integer> hardDiskSpace, int segmentLength) {
        List<Integer> segment = new ArrayList<>();
        int index = segmentLength;
        while (index < numComputer) {
            List<Integer> list = hardDiskSpace.subList(index - segmentLength, index);
            segment.add(Collections.min(list));
            index++;
        }
        return Collections.max(segment);
    }
}
