...
/**
 * Created by Jim Z on 11/21/20 22:20
 *
...
public class LC1219_PathwithMaximumGold {
    public int getMaximumGold(int[][] grid) {
        // get col & rol length to avoid repeat calls
    ...
    private int findMaxGold(int[][] grid, int m, int n, int r, int c) {
        ...
        int origin = grid[r][c];
        grid[r][c] = 0; // mark (r,c) as visited
        int maxGold = 0;
        for (int i = 0; i < 4; i ++) {
            maxGold = Math.max(maxGold, findMaxGold(grid, m, n, r + DIR[i][0], c + DIR[i][1]));
        }
